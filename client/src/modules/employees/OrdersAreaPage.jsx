...
function CancelOrderModal({ order, onClose, onConfirm }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <form className="w-full max-w-lg rounded-card bg-white p-6 shadow-drawer" onSubmit={async (event) => { event.preventDefault(); setBusy(true); setError(""); try { await onConfirm(prepared ? form : {}); } catch (failure) { setError(failure.message); } finally { setBusy(false); } }}>

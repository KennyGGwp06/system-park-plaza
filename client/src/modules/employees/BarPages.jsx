...
import { AlertCircle, Boxes, CheckCircle2, ClipboardList, Clock, Droplets, Play, Save, Trash2, Wine } from "lucide-react";
...
export function BarDashboard() {
  ...
  return <main className="space-y-6 py-5">
    ...
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <Metric label="Estado" value={session.statusLabel || session.status} help={session.areaName || "Bar"} />
...
export function BarOrdersPage() {
  ...
  const next = { PENDIENTE: "PREPARANDO", PREPARANDO: "LISTO", LISTO: "ENTREGADO" };
  ...
  return <main className="space-y-6 py-5">
    ...
    {!orders.length ? <Empty title="No hay bebidas pendientes" text="Los nuevos pedidos autorizados aparecerán aquí." /> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{orders.map((order) => <article className="rounded-card border border-park-border bg-white p-5 shadow-card" key={order.id}>
      ...
      {order.status === "PENDIENTE" ? <div className="grid grid-cols-2 gap-2"><Button onClick={() => change(order, "PREPARANDO")} disabled={busy === order.id}>{busy === order.id ? "Procesando..." : "Aceptar"}</Button><Button variant="secondary" onClick={() => change(order, "CANCELADO")} disabled={busy === order.id}>Rechazar</Button></div> : <Button className="w-full" onClick={() => change(order, next[order.status])} disabled={busy === order.id}>{busy === order.id ? "Procesando..." : order.status === "PREPARANDO" ? "Marcar listo" : "Entregar"}</Button>}
    </article>)}</div>}
  </main>;
}
...
export function BarRecipesPage() {
  ...
  return <main className="space-y-6 py-5"><Header icon={Droplets} eyebrow="Bar · solo lectura" title="Manual y medidas" description="Recetas, insumos y porciones estándar definidos por el Superadmin." />
    {!recipes.length ? <Empty title="No hay recetas publicadas" text="Solicita al Superadmin completar la ficha técnica." /> : <div className="grid gap-4 lg:grid-cols-2">{recipes.map((recipe) => <article className="rounded-card border border-park-border bg-white p-5 shadow-card" key={recipe.id || recipe.menuItemId || recipe.name}><h2 className="font-black text-park-dark">{recipe.name || recipe.menuItemName}</h2><p className="mt-1 text-sm text-park-muted">{recipe.yield || recipe.portionName || "Porción estándar"}</p><div className="mt-4 space-y-2">{(recipe.ingredients || recipe.lines || []).map((line, index) => <div className="flex justify-between border-b border-park-border py-2 text-sm" key={index}><span>{line.productName || line.name}</span><b>{line.quantity} {line.unitSymbol || line.unit || ""}</b></div>)}</div></article>)}</div>}
  </main>;
}

export function BarInventoryPage() {
  const { session, sessionsRequest, detailRequest } = useBarSession();
  const [tab, setTab] = useState("stock"), [busy, setBusy] = useState(false), [message, setMessage] = useState("");

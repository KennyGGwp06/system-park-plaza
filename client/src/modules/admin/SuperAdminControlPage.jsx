...
export function SuperAdminControlPage() {
  ...
  const roomsWithUsage = (state.rooms || []).filter((room) => room.usage);
  const roomsInUse = roomsWithUsage.filter((room) => room.usage.state === "EN_USO");
  const reservedRooms = roomsWithUsage.filter((room) => room.usage.state === "RESERVADA");
  ...
  return <div className="reception-command space-y-6 pb-10">
    ...
    <section className="rounded-card border border-park-border bg-white p-5 shadow-card">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.16em] text-emerald-700">Ocupación hotelera</p><h2 className="text-xl font-black text-park-dark">Habitaciones registradas por clientes</h2><p className="text-sm text-park-muted">Las reservas del cliente aparecen aquí y se convierten en “En uso” cuando Recepción realiza el check-in.</p></div><p className="text-sm font-bold text-park-muted">{roomsInUse.length} en uso · {reservedRooms.length} reservada(s)</p></div>
      {roomsWithUsage.length ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{roomsWithUsage.map((room) => <article className={`rounded-xl border p-4 ${room.usage.state === "EN_USO" ? "border-blue-200 bg-blue-50" : "border-amber-200 bg-amber-50"}`} key={room.id}><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.12em] text-park-muted">Habitación</p><strong className="text-2xl text-park-dark">{room.number}</strong></div><span className="rounded-full bg-white/80 px-2.5 py-1 text-xs font-black text-park-dark">{room.usage.label}</span></div><p className="mt-3 font-black text-park-dark">{room.usage.clientName}</p><p className="mt-1 text-sm text-park-muted">{room.usage.reservationCode || "Reserva sin código"} · Entrada {shortDate(room.usage.checkIn)}</p></article>)}</div> : <p className="rounded-xl bg-park-bg px-4 py-5 text-sm text-park-muted">Aún no hay habitaciones reservadas ni en uso.</p>}
    </section>
...
function shortDate(value) { const [year, month, day] = String(value || "").slice(0, 10).split("-"); return year && month && day ? `${day}/${month}/${year}` : "por confirmar"; }

...
function UserModal({ modal, roles, onClose, onSave, saving }) {
  ...
  const [form, setForm] = useState({ firstName: target?.firstName || "", lastName: target?.lastName || "", email: target?.email || "", documentNumber: target?.documentNumber || "", phone: target?.phone || "", birthDate: dateInputValue(target?.birthDate), photoUrl: target?.photoUrl || "", position: target?.position || "", hireDate: dateInputValue(target?.hireDate), username: target?.username || "", roleId: target?.roleId || target?.role?.id || "", status: target?.status || "ACTIVO", password: "" });
  function update(key, value) { setForm((state) => ({ ...state, [key]: value })); }
  function submit(event) { event.preventDefault(); onSave(modal.type === "reset" ? userPayload(target, { password: form.password, status: "ACTIVO" }) : form, target); }
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4 backdrop-blur-sm animate-in fade-in duration-200">
      <form className="max-h-[90vh] w-full max-w-3xl overflow-auto rounded-card bg-white p-6 shadow-drawer" onSubmit={submit}>
        ...
        {modal.type === "create" ? <p className="mt-3 rounded-card bg-park-green-soft p-3 text-sm font-semibold text-park-green">Registra los datos y entrega una contraseña temporal exclusiva al nuevo trabajador.</p> : null}

...
export function AdminMaintenancePage({ view = "resumen" }) {
  ...
  return <div className="space-y-5">
    <PageHeader eyebrow="Operación / Soporte externo" title={pageTitle(view)} description="Registra el problema, coordina al proveedor externo y conserva la trazabilidad hasta su cierre." actions={<Button icon={Plus} onClick={() => setCreating(true)}>Nueva incidencia</Button>} />
    ...
    {visible.length ? <section className="overflow-hidden rounded-card border border-park-border bg-white shadow-card"><div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead className="bg-park-bg text-xs uppercase text-park-muted"><tr><th className="px-4 py-3">Código</th><th>Ubicación</th><th>Problema</th><th>Prioridad</th><th>Proveedor</th><th>Visita</th><th>Estado</th><th></th></tr></thead><tbody className="divide-y divide-park-border">{visible.map((item) => <tr key={item.id}><td className="px-4 py-3 font-black">{item.code}</td><td>{item.location || item.area}</td><td className="max-w-xs">{item.description}</td><td><StatusBadge value={item.priority}/></td><td>{item.contractorName || "Por coordinar"}</td><td>{item.visitDate ? new Date(`${item.visitDate}T12:00:00`).toLocaleDateString("es-PE") : "Sin fecha"}</td><td><StatusBadge value={item.status}/></td><td className="pr-4"><Button icon={Eye} size="sm" variant="secondary" onClick={() => setSelected(item)}>Gestionar</Button></td></tr>)}</tbody></table></div></section> : <EmptyState title="Sin incidencias en esta vista" description="Cuando se registre un problema aparecerá aquí para su seguimiento."/>}
    {creating ? <IncidentForm form={form} setForm={setForm} onClose={() => setCreating(false)} onSubmit={createIncident}/> : null}
    {selected ? <IncidentDetail report={selected} onClose={() => setSelected(null)} onSaved={async () => { setSelected(null); setMessage("Seguimiento actualizado correctamente."); await reload(); }}/>: null}
  </div>;
}
...
function IncidentForm({ form, setForm, onClose, onSubmit }) {
  ...
  return <Modal title="Registrar incidencia" subtitle="No se asigna un empleado de mantenimiento: Recepción o Administración coordinará a un proveedor externo." onClose={onClose}><form className="grid gap-4" onSubmit={onSubmit}><div className="grid gap-4 sm:grid-cols-2"><Select label="Tipo" {...field("type")}><option>DANO_INFRAESTRUCTURA</option><option>DANO_EQUIPO</option><option>FALLA_ELECTRICA</option><option>FALLA_SANITARIA</option><option>OTRO</option></Select><Select label="Prioridad" {...field("priority")}><option>BAJA</option><option>MEDIA</option><option>ALTA</option><option>CRITICA</option></Select><Input label="Área" required {...field("area")}/><Input label="Ubicación exacta" required placeholder="Ej. Habitación 204" {...field("location")}/></div><label className="block"><span className="mb-1.5 block text-sm font-semibold">Descripción</span><textarea className="min-h-28 w-full rounded-input border border-park-border p-3 outline-none focus:border-park-green" required value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })}/></label><div className="flex justify-end gap-2"><Button type="button" variant="secondary" onClick={onClose}>Cancelar</Button><Button type="submit">Registrar y avisar</Button></div></form></Modal>;
}

function IncidentDetail({ report, onClose, onSaved }) {
  const [draft, setDraft] = useState({ contractorName: report.contractorName || "", contractorPhone: report.contractorPhone || "", visitDate: report.visitDate || "", estimatedCost: report.estimatedCost || "", notes: report.notes || "", status: report.status });
  const [busy, setBusy] = useState(false);
  const field = (name) => ({ value: draft[name], onChange: (event) => setDraft({ ...draft, [name]: event.target.value }) });
  async function save(status = draft.status) { setBusy(true); try { await api(`/reports/${report.id}/status`, { method: "PATCH", body: { ...draft, estimatedCost: Number(draft.estimatedCost || 0), status } }); await onSaved(); } finally { setBusy(false); } }
  return <Modal title={`${report.code} · Gestionar soporte`} subtitle={`${report.location || report.area} — ${report.description}`} onClose={onClose}><div className="mb-4 flex gap-2"><StatusBadge value={report.priority}/><StatusBadge value={report.status}/></div><div className="grid gap-4 sm:grid-cols-2"><Input label="Proveedor o técnico externo" placeholder="Nombre o empresa" {...field("contractorName")}/><Input label="Teléfono" type="tel" placeholder="999 999 999" {...field("contractorPhone")}/><Input label="Fecha de visita" type="date" {...field("visitDate")}/><Input label="Costo estimado (S/)" min="0" step="0.01" type="number" {...field("estimatedCost")}/></div><label className="mt-4 block"><span className="mb-1.5 block text-sm font-semibold">Notas de coordinación</span><textarea className="min-h-24 w-full rounded-input border border-park-border p-3" {...field("notes")}/></label>{draft.contractorPhone ? <a className="mt-4 inline-flex items-center gap-2 font-bold text-park-green" href={`tel:${draft.contractorPhone}`}><Phone size={17}/>Llamar al proveedor</a> : null}<div className="mt-5 flex flex-wrap justify-end gap-2"><Button variant="secondary" onClick={onClose}>Cancelar</Button>{report.status === "ABIERTO" ? <Button disabled={busy} onClick={() => save("EN_REVISION")}>Guardar e iniciar seguimiento</Button> : null}{report.status === "EN_REVISION" ? <Button disabled={busy} onClick={() => save("RESUELTO")}>Marcar como resuelta</Button> : null}{report.status === "RESUELTO" ? <Button disabled={busy} onClick={() => save("RESUELTO")}>Guardar cambios</Button> : null}</div></Modal>;
}

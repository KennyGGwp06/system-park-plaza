...
function menuAvailableForService(item, serviceCode) {
  if (item?.salesEnabled !== true || item?.available !== true) return false;
  const services = Array.isArray(item.availableFor) ? item.availableFor : String(item.availableFor || "").split(/[\s,|]+/);
  const normalized = services.map((value) => String(value).toUpperCase()).filter(Boolean);
  return !normalized.length || normalized.includes(String(serviceCode || "").toUpperCase());
}
...
function LookoutFlowEnhanced({ service, catalog, hasExistingParking, onBack, onCheckout }) {
  ...
  const lookoutMenu = (catalog.restaurantMenu || []).filter((item) => menuAvailableForService(item, "MIRADOR")); const selectedDay = days.find((item) => item.date === date) || days[0]; const preorderItems = lookoutMenu.filter((item) => cart[item.id]).map((item) => ({ menuItemId: item.id, name: item.name, quantity: cart[item.id], price: item.price })); const menuTotal = preorderItems.reduce((sum, item) => sum + item.price * item.quantity, 0); const base = Number(plan?.price || 0) * people; const extrasTotal = extras.reduce((sum, item) => sum + item.price, 0) + menuTotal; const parkingTotal = vehicles.reduce((sum, item) => sum + Number(item.price || 0), 0); const total = base + extrasTotal + parkingTotal;
  function next() { onCheckout({ service, planCode: plan.code, planName: plan.name, date, checkIn: date, checkOut: date, slot, people, adults: people, children: 0, extras, parking: vehicles[0] || null, vehicles, preferences: {}, preorderItems, nights: 1, base, extrasTotal, parkingTotal, total }); }

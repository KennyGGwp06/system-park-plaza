...
function menuAvailableForService(item, serviceCode) {
  ...
  return item?.salesEnabled === true && item?.available === true && services.map((value) => String(value).toUpperCase()).includes(String(serviceCode).toUpperCase());
}
...
function LookoutFlow({ service, catalog, hasExistingParking, onBack, onCheckout }) {
  ...
  useEffect(() => { api(`/public/availability/MIRADOR?from=${calendarFrom}`).then(setDays); }, [calendarFrom]); const selectedDay = days.find((item) => item.date === date) || days[0]; const lookoutMenu = (catalog.restaurantMenu || []).filter((item) => menuAvailableForService(item, "MIRADOR")); const preorderItems = lookoutMenu.filter((item) => cart[item.id]).map((item) => ({ menuItemId: item.id, name: item.name, quantity: cart[item.id], price: item.price })); const menuTotal = preorderItems.reduce((sum, item) => sum + item.price * item.quantity, 0); const base = Number(plan?.price || 0) * people; const extrasTotal = extras.reduce((sum, item) => sum + item.price, 0) + menuTotal; const parkingTotal = vehicles.reduce((sum, item) => sum + Number(item.price || 0), 0); const total = base + extrasTotal + parkingTotal;

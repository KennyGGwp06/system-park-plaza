...
export function App() {
  ...
  return (
    <div className={`customer-app ${screen === "welcome" ? "welcome-active" : ""}`}>
      ...
      {screen === "recover" ? <RecoverReservation documentNumber={recoveryDocument} setDocumentNumber={setRecoveryDocument} onBack={() => go("home")} onDone={(value) => { activateClient(value); go("reservations"); }} /> : null}
...
function RecoverReservation({ onBack, onDone, documentNumber, setDocumentNumber }) {
  ...
  return <Page title="Verificación de reserva" subtitle="Confirma tu identidad para ver tus reservas, saldos, accesos y comprobantes." onBack={onBack}><form className="card form" onSubmit={submit}><Info icon={QrCode} title="Tu información en un solo lugar" text="No se crea otro QR: verificaremos las reservas y accesos asociados a tu documento."/><Field label="DNI o carnet de extranjería" value={documentNumber} onChange={setDocumentNumber}/>{error ? <p className="error">{error}</p> : null}<button className="primary wide" disabled={busy}>{busy ? "Verificando…" : "Verificar mis reservas"}</button><small className="center">¿Aún no reservaste? Vuelve a experiencias y elige el servicio que deseas vivir.</small></form></Page>;
}

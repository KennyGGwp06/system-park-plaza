...
import { ArrowLeft, ArrowRight, Banknote, BedDouble, CalendarDays, Car, Check, ChevronRight, ClipboardList, Clock3, ConciergeBell, CreditCard, Download, Eye, EyeOff, HelpCircle, Home, LockKeyhole, Minus, Plus, QrCode, ShoppingBag, Smartphone, Sparkles, SunMedium, Users, Waves, Map, BookOpen, Wifi, Coffee, FileText, CheckCircle2, Utensils, ChefHat, Bike } from "lucide-react";
...
export function App() {
  ...
  return (
    <div className={`customer-app ${screen === "welcome" ? "welcome-active" : ""} ${screen === "experience" ? "qr-experience-active" : ""} ${screen === "requests" ? "concierge-active" : ""} ${screen === "success" && ["PISCINA", "HOSPEDAJE"].includes(selection?.service?.code) ? "payment-success-active" : ""}`}>
      ...
      {screen === "recover" ? <RecoverReservation documentNumber={recoveryDocument} setDocumentNumber={setRecoveryDocument} password={recoveryPassword} setPassword={setRecoveryPassword} onBack={() => go(client ? "home" : "welcome")} onDone={(value) => { activateClient(value); setRecoveryPassword(""); go("reservations"); }} /> : null}
...
function RecoverReservation({ onBack, onDone, documentNumber, setDocumentNumber, password, setPassword }) {
  const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [visible, setVisible] = useState(false);
  async function submit(event) { event.preventDefault(); setBusy(true); setError(""); try { const result = await request("/public/recover", { method: "POST", body: { documentNumber, password } }); localStorage.setItem("pp_customer_token", result.token); localStorage.setItem("pp_customer_client", JSON.stringify(result.client)); onDone(result.client); } catch (cause) { setError(cause.message); } finally { setBusy(false); } }
  return <main className="reservation-verification"><img className="reservation-verification-image" src="/images/experiences/hospedaje.webp" alt="Recepción de Park Plaza"/><div className="reservation-verification-veil"/><header className="reservation-verification-top"><button type="button" onClick={onBack}><ArrowLeft/>Volver</button><div className="reservation-verification-brand"><img src="/brand/park-plaza-mark.svg" alt="Park Plaza"/><span>PARK PLAZA<small>PUCALLPA</small></span></div><span>PP</span></header><section className="reservation-verification-copy"><small>EXPERIENCIA PARK PLAZA</small><h1>Verificación de reserva</h1><p>Confirma tu documento y la contraseña de tu cuenta para proteger tus saldos, accesos y comprobantes.</p></section><form className="reservation-verification-card" onSubmit={submit}><div className="reservation-verification-notice"><LockKeyhole/><div><b>Acceso protegido</b><p>Verificaremos tu identidad antes de mostrar tus experiencias.</p></div></div><label><span>DNI o carnet de extranjería</span><input required value={documentNumber} autoComplete="username" inputMode="numeric" onChange={(event) => setDocumentNumber(event.target.value)}/></label><label className="reservation-verification-password"><span>Contraseña de tu cuenta</span><div><input required type={visible ? "text" : "password"} value={password} autoComplete="current-password" onChange={(event) => setPassword(event.target.value)}/><button type="button" aria-label={visible ? "Ocultar contraseña" : "Mostrar contraseña"} onClick={() => setVisible((value) => !value)}>{visible ? <EyeOff/> : <Eye/>}</button></div></label>{error ? <p className="error">{error}</p> : null}<button className="reservation-verification-submit" disabled={busy}><LockKeyhole/>{busy ? "Verificando…" : "Verificar mis reservas"}</button><p className="reservation-verification-help">Usa la misma contraseña con la que creaste tu cuenta. ¿Aún no reservaste? <button type="button" onClick={onBack}>Elige una experiencia</button></p></form></main>;
}
...
function Identify({ onBack, onDone, onRecover, form, setForm, reservationFlow, initialMode = "register" }) {
  ...
  const [loginForm, setLoginForm] = useState({ documentNumber: "", password: "" });
  ...
  async function submit(event) {
    ...
    if (registrationPassword.length < 8 || !/[A-Za-z]/.test(registrationPassword) || !/\d/.test(registrationPassword)) return setError("Crea una contraseña de 8 caracteres como mínimo, con una letra y un número.");
    if (registrationPassword !== confirmPassword) return setError("Las contraseñas no coinciden.");
    setBusy(true); setError("");
    try {
      const result = await request("/public/identify", { method: "POST", body: { ...form, documentNumber, firstName, lastName, phone, email, password: registrationPassword } });
      localStorage.setItem("pp_customer_token", result.token);
  ...
  async function login(event) {
    ...
    if (!documentNumber || !loginForm.password) return setError("Ingresa tu documento y contraseña.");
    setBusy(true); setError("");
    try {
      const result = await request("/public/login", { method: "POST", body: { identifier: documentNumber, password: loginForm.password } });
      localStorage.setItem("pp_customer_token", result.token);
      localStorage.setItem("pp_customer_client", JSON.stringify(result.client));
      onDone(result.client);
  ...
  const sceneSubtitle = loginMode ? "Reservas, pagos, pedidos y accesos. Todo vuelve a ti con una sola contraseña." : "Crea tu cuenta y convierte cada visita en una experiencia hecha a tu medida.";
  ...
  return <main className={`identity-page identity-${mode}`}>
    ...
    <section className="identity-panel">
      <div className="identity-shell">
        <div className="identity-mode-switch" role="tablist" aria-label="Acceso a Park Plaza"><i className={mode}/><button type="button" role="tab" aria-selected={loginMode} className={loginMode ? "active" : ""} onClick={() => changeMode("login")}>Iniciar sesión<small>Ya tengo cuenta</small></button><button type="button" role="tab" aria-selected={!loginMode} className={!loginMode ? "active" : ""} onClick={() => changeMode("register")}>Crear cuenta<small>Primera visita</small></button></div>
        <div className="identity-form-stage" key={mode}>
          {loginMode ? <form className="identity-form identity-login-form" onSubmit={login}>
            <header><small>ACCESO PRIVADO</small><h2>Hola de nuevo</h2><p>Ingresa a tu universo Park Plaza.</p></header>
            <div className="identity-login-emblem"><LockKeyhole/><div><b>Tu espacio está protegido</b><small>Usa el documento y la contraseña con los que creaste tu cuenta.</small></div></div>
            <div className="identity-fields"><Field label="Correo o documento de identidad" value={loginForm.documentNumber} maxLength={120} autoComplete="username" onChange={(documentNumber) => setLoginForm({ ...loginForm, documentNumber })}/><PasswordField label="Contraseña" value={loginForm.password} onChange={(password) => setLoginForm({ ...loginForm, password })} visible={showPassword} onToggle={() => setShowPassword((value) => !value)}/></div>
            ...
            <div className="identity-form-links"><button type="button" onClick={onRecover}>¿Olvidaste tu contraseña?</button><button type="button" onClick={() => changeMode("register")}>Crear una cuenta</button></div>
            ...
            <div className="form-divider"><span>o crea tu acceso personal</span></div>
  ...
  return <main className="concierge-studio">
    ...
    <section className="concierge-panel" aria-label="Nueva solicitud de conserjería">
      ...
      <div className="concierge-note">
        ...
        <button className="concierge-submit" onClick={send} disabled={busy}>{busy ? "Enviando solicitud…" : <>Enviar solicitud <ArrowRight size={20}/></>}</button>
      </div>
    </section>
  </main>; 
}
...
function PasswordField({ label, value, onChange, visible, onToggle }) { return <label className="field password-field"><span>{label}</span><div><input required type={visible ? "text" : "password"} value={value} autoComplete="current-password" onChange={(event) => onChange(event.target.value)}/><button type="button" aria-label={visible ? "Ocultar contraseña" : "Mostrar contraseña"} onClick={onToggle}>{visible ? <EyeOff/> : <Eye/>}</button></div></label>; }
